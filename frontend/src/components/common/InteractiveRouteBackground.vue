<template>
  <div
    ...
    :class="{ 'is-dark': isDark, 'is-bursting': burstActive }"
    aria-hidden="true"
  >
...
function buildScene() {
  ...
  const particleCount = Math.min(680, Math.max(320, Math.round((width * height) / 2500)))
...
function drawBackground(context: CanvasRenderingContext2D) {
  ...
  aurora.addColorStop(0, `rgba(${tertiary}, ${themeMix > 0.55 ? 0.16 : 0.11})`)
  aurora.addColorStop(0.42, `rgba(${secondary}, ${themeMix > 0.55 ? 0.09 : 0.055})`)
  ...
  pointerGlow.addColorStop(0, `rgba(${primary}, ${0.07 + pointer.strength * 0.11})`)
  pointerGlow.addColorStop(0.34, `rgba(${secondary}, ${0.035 + pointer.strength * 0.05})`)
  pointerGlow.addColorStop(0.74, `rgba(${primary}, ${0.012 + pointer.strength * 0.024})`)
...
function drawDepthRings(context: CanvasRenderingContext2D) {
  ...
  for (let index = 0; index < 10; index += 1) {
    ...
    const alpha = (0.018 + depth * 0.018 + pointer.strength * 0.012) * (themeMix > 0.55 ? 1 : 0.58)
...
function drawFlowLanes(context: CanvasRenderingContext2D) {
  ...
  for (const [index, lane] of lanes.entries()) {
    ...
    gradient.addColorStop(0.18, `rgba(${laneColor}, ${0.045 + themeMix * 0.035})`)
    gradient.addColorStop(0.5, `rgba(${white}, ${themeMix > 0.55 ? 0.16 : 0.24})`)
    gradient.addColorStop(0.82, `rgba(${laneColor}, ${0.055 + pointer.strength * 0.045})`)
    ...
    context.strokeStyle = `rgba(${laneColor}, ${0.08 + pointer.strength * 0.08})`
...
function drawCells(context: CanvasRenderingContext2D) {
  ...
  const baseAlpha = themeMix > 0.55 ? 0.048 : 0.02
  ...
  for (const cell of cells) {
    ...
    const target = Math.min(1, focus * brokenMask + sweep * 0.12 + shimmer)
    ...
    if (cell.seed > 0.2) {
      context.fillStyle = `rgba(${pixel}, ${ambient * (themeMix > 0.55 ? 0.72 : 0.18)})`
    ...
    const alpha = 0.08 + cell.energy * (themeMix > 0.55 ? 0.55 : 0.3)
    ...
    context.fillStyle = `rgba(${tone}, ${cell.energy * (themeMix > 0.55 ? 0.085 : 0.035)})`
...
function drawParticles(context: CanvasRenderingContext2D, delta: number) {
  ...
  for (const particle of particles) {
    ...
    const size = particle.size + particle.z * 1.8 + force * 4.6
    const alpha = (0.055 + particle.z * 0.17 + force * 0.35) * (themeMix > 0.55 ? 0.8 : 0.45)
...
function drawBottomSignal(context: CanvasRenderingContext2D) {
  ...
  const gradient = context.createLinearGradient(0, baseY, width, baseY)
  gradient.addColorStop(0, 'rgba(0, 0, 0, 0)')
  gradient.addColorStop(0.14, `rgba(${primary}, ${0.14 + themeMix * 0.1})`)
  gradient.addColorStop(0.5, `rgba(${secondary}, ${0.075 + pointer.strength * 0.08})`)
  gradient.addColorStop(0.86, `rgba(${primary}, ${0.14 + themeMix * 0.1})`)
  gradient.addColorStop(1, 'rgba(0, 0, 0, 0)')
  ...
  for (let x = 0; x < width; x += 24) {
    ...
    context.fillStyle = `rgba(${primary}, ${0.075 + seed * 0.13})`
...
function drawThemeShock(context: CanvasRenderingContext2D, now: number) {
  if (burstStarted < 0) return

  const progress = Math.min(1, (now - burstStarted) / burstDuration)
  if (progress >= 1) {
    burstStarted = -1
    return
  }
  ...
  const secondary = color('secondary')
  const white = color('white')
  const x = burstOrigin.x * width
  const y = burstOrigin.y * height
  const radius = eased * Math.max(width, height) * 1.1
  const alpha = (1 - progress) * 0.62

  context.save()
  context.globalCompositeOperation = 'lighter'
  context.strokeStyle = `rgba(${white}, ${alpha * 0.6})`
  context.lineWidth = Math.max(2, 18 * (1 - progress))
  context.beginPath()
  context.arc(x, y, radius, 0, Math.PI * 2)
  context.stroke()

  context.strokeStyle = `rgba(${primary}, ${alpha})`
  context.lineWidth = Math.max(1, 9 * (1 - progress))
  context.beginPath()
  context.arc(x, y, radius * 0.74, 0, Math.PI * 2)
  context.stroke()

  for (let index = 0; index < 46; index += 1) {
    const seed = rand(index * 9.7)
    const angle = seed * Math.PI * 2
    const sparkleRadius = radius * (0.42 + rand(index * 18.4) * 0.62)
    const px = x + Math.cos(angle) * sparkleRadius
    const py = y + Math.sin(angle) * sparkleRadius
    const size = 2 + seed * 8 * (1 - progress)
    context.fillStyle = `rgba(${index % 2 ? primary : secondary}, ${alpha * seed})`
...
function render(now = 0) {
  ...
  themeMix = lerp(themeMix, props.isDark ? 1 : 0, burstActive.value ? 0.32 : 0.14)
...
.route-theme-flash {
  background:
    radial-gradient(circle at 50% 42%, rgba(255, 255, 255, 0.94), rgba(45, 212, 191, 0.34) 24%, rgba(37, 99, 235, 0.26) 43%, transparent 69%);
  opacity: 0;
  transform: scale(0.16);
}

.route-background.is-bursting .route-theme-flash {
  animation: route-theme-impact 0.98s cubic-bezier(0.16, 1, 0.3, 1);
}

.route-background.is-dark .route-theme-flash {
  background:
    radial-gradient(circle at 50% 42%, rgba(45, 212, 191, 0.9), rgba(96, 165, 250, 0.48) 24%, rgba(14, 165, 233, 0.28) 46%, transparent 70%);
}
